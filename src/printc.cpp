    ...
    QObject(parent)
{
    ...
    m_data -> getInfo(info);
    mtextInfo = info.name + "\n";
    if(!info.address1.isEmpty()) mtextInfo += tr("") + info.address1 + "\n";
    if(!info.address2.isEmpty()) mtextInfo += tr("") + info.address2 + "\n";
    if(!info.address3.isEmpty()) mtextInfo += tr("") + info.address3 + "\n";
    if(!info.zipCode.isEmpty() && !info.city.isEmpty()) mtextInfo += tr("") + info.zipCode + " " + info.city + "\n";
...
void Printc::load_parameters(QPrinter *printer, QPainter &painter) {
    ...
    // ATTENTION: l'impression physique et PDF n'ont pas le même maximum DPI
...
void Printc::load_customerInfos() {
    ...
    mtextidentity +=  m_cus -> getLastName()+" "+m_cus -> getFirstName()+'\n';
    // pas de numéro de téléphone dans la facture
...
void Printc::print_content(QPainter &painter, QRectF &rect, itemList Ilist, int &itemPrinted, int page, int NbOfpage) {
    ...
    //QUANTITE 8%
    rect = painter.fontMetrics().boundingRect(mLeft+SPACE_BORDER+mwUtil*pos,rect.top(), mwUtil*WIDTH_QTE,0, Qt::AlignLeft, tr("Qté")  );
    pos+=WIDTH_QTE;
    painter.drawText( rect, tr("Qté") );
    //TOTAL 12%
    QString stt;
    if(m_data->getIsTax())
...
void Printc::on_paintPrinterProposal(QPrinter *printer) {
    ...
    rect = painter.fontMetrics().boundingRect(mLeft+(mwUtil*0.62)+SPACE_BORDER, /*rect.bottom()*/mpageRect.height() - mBottom - mpageRect.height()*0.15 /* + SPACE_BORDER*16 */, 0, 0, Qt::AlignLeft, text );
    painter.drawText(rect, text);
